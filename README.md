...
4. Deploy. After the first deploy succeeds, run migrations + seed against the
   ...
   ```
5. Visit the Vercel URL — that's your Live URL for the submission form.

## Loom video outline (5–10 min) — suggested structure

1. (30s) Which role + track you picked and why, which 4 features.
2. (2 min) Live demo: search/filter/sort/paginate → open a detail page → add 2–3
   colleges to compare → sign up → save a college → view saved list.
3. (3 min) Architecture walkthrough: Prisma schema (normalized tables, indexes),
   the `/api/colleges` query-building logic, why filtering happens server-side.
4. (2 min) Decisions & tradeoffs: Credentials auth vs OAuth, comparison snapshot
   vs live re-fetch, what you'd do with more time (see Known Limitations above).
5. (30s) Edge cases handled: empty search results, pagination bounds, duplicate
   signup emails, comparing fewer than 2 or more than 4 colleges.
